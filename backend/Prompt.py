class Prompt : 
    def __init__(self,choice="organize", **kwargs) : 
        # Init prompt string
        self.prompt = ""
        self.option = choice
        ...
        if choice == "organize" :
            if(kwargs.get("timetable") and kwargs.get("tasks")) :
                self.timetable, self.tasks = kwargs.get("timetable", "tasks")
            else:
    ...
    def getAll(self) :
        return {
            "organizer" : self.organize("tasks", "timetable"),
            "planner ": self.plan("tasks")
        }
    ...
    def organize(self,tasks, timetable) : 
        ...
        - A non-scholar task would be moire suitable after the courses and before the homework to help make pauses.
    ...
    def plan(self,tasks) :
        ...
        - Ensure that tasks are interpolated optimally while respecting their duration.
        - A non-scholar task would be suitable as a break between two long run of tasks.
